...
try:
    file, path, description = imp.find_module('sru_cpu_impl', [path])
    with file:
        SRU_CPU_kernel = imp.load_module('sru_cpu_impl', file, path, description)
except ImportError:
    SRU_CPU_kernel = None
SRU_GPU_kernel = None
...
def _lazy_load_cpu_kernel():
    ...
    try:
        ...
        SRU_CPU_kernel = load(name='sru_cpu_impl', sources=[cpu_source],
                              extra_cflags=['-IC:/Program Files (x86)/Microsoft Visual Studio 14.0/VC/include',
                                            '-IC:/Program Files (x86)/Windows Kits/10/Include/10.0.10240.0/ucrt',
                                            '-IC:/Users/Howie_Laem/AppData/Local/Programs/Common/Microsoft/'
                                            'Visual C++ for Python/9.0/WinSDK/Include'],
                              extra_ldflags=['/LIBPATH:C:/Program Files (x86)/Microsoft Visual Studio '
                                             '14.0/VC/lib/amd64',
                                             '/LIBPATH:C:/Users/Howie_Laem/AppData/Local/Programs/Common/Microsoft/'
                                             'Visual C++ for Python/9.0/WinSDK/Lib/x64',
                                             '/LIBPATH:C:/Program Files (x86)/Windows Kits/10/Lib/10.0.10240.0/'
                                             'ucrt/x64'],
...
class SRU(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(num_layers):
            ...
            if layer_norm:
                self.ln_lst.append(nn.LayerNorm(self.out_size, eps=1e-6))
